...
curl -o /usr/local/bin/update-config.sh https://raw.githubusercontent.com/legendleap/hetzner-cloud-init/bocs/update-config.sh

chmod +x /usr/local/bin/update-config.sh

# ufw allow proto tcp from any to any port 22
ufw allow proto tcp from any to any port 22,110,995,143,993,25,465,587 #for email node behind proxy
